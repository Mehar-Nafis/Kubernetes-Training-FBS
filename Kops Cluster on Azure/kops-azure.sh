...
cleanup_on_failure() {
  ...
  # 1. Delete ONLY this run's cluster resources, scoped by name.
  #    We deliberately DO NOT run 'kops delete cluster': on Azure kops deletes at
  #    the RESOURCE-GROUP level and will try to delete every NSG/ASG/etc. in the
  #    shared RG — including OTHER students' resources — looping forever on the
  #    ones it can't (InUseNetworkSecurityGroupCannotBeDeleted). Instead we delete
  #    only resources whose name carries THIS run's unique cluster signature
  #    ("<username>-<date>.k8s.local"), so it can never touch anyone else.
  if [ "$CLUSTER_CREATED" -eq 1 ]; then
    echo ">>> Deleting resources for cluster: $CLUSTER_NAME (name-scoped, NOT 'kops delete cluster') ..."
    # VM scale sets first so their NICs/disks/identities release the networking.
    az resource list -g "$RG_NAME" \
      --query "[?contains(name,'${CLUSTER_NAME}') && type=='Microsoft.Compute/virtualMachineScaleSets'].id" -o tsv 2>/dev/null \
      | xargs -r -n1 -I{} az resource delete --ids {} >/dev/null 2>&1 || true
    # Then retry-delete the rest (handles subnet-before-vnet style dependencies).
    for _attempt in 1 2 3 4 5 6; do
      _left=$(az resource list -g "$RG_NAME" \
                --query "[?contains(name,'${CLUSTER_NAME}')].id" -o tsv 2>/dev/null || true)
      [ -z "$_left" ] && break
      echo "    ... $(echo "$_left" | grep -c .) resource(s) left for $CLUSTER_NAME, deleting (attempt $_attempt/6)"
      echo "$_left" | xargs -r -n1 -P4 -I{} az resource delete --ids {} >/dev/null 2>&1 || true
      sleep 8
    done
    _left=$(az resource list -g "$RG_NAME" --query "[?contains(name,'${CLUSTER_NAME}')].id" -o tsv 2>/dev/null || true)
    [ -n "$_left" ] && echo "    (some resources remain — run: USERNAME=$lower_username FORCE=true ./kops-azure-cleanup.sh)"
  fi
